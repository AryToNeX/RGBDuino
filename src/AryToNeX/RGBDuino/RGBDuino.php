...
exec("notify-send -u normal -i arduino \
...
pcntl_signal(SIGINT, function() use ($status){
    $status->setShouldExit(1);
});

// start with default color
$status->getArduino()->sendColorArray(color\Color::fromHexToRgb($status->getConfig()->getValue("defaultColor") ?? "FFFFFF"));
